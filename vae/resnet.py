...
from vae.utils import activation, normalization
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if resample == "down":
            ...
            self.resample = nn.Upsample(scale_factor=resample_factor, mode="trilinear")
            self.conv1 = nn.ConvTranspose2d(
                in_channels, out_channels, kernel_size=kernel_size, padding=padding
            )
            output_padding = tuple(
                2 * p + s - k
                for (p, s, k) in zip(padding, resample_factor, kernel_size)
            )
...
if __name__ == "__main__":
    ...
    block = ResBlock(
        1, 2, resample="down", resample_factor=(2, 2), norm_kwargs={"num_groups": 1}
    )
